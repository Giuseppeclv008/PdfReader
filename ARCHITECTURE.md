...
├── core/
│   └── pdf_converter.py        # convert_one() / convert_folder() — pure orchestration
├── menu/
...
| `core/pdf_converter.py` | Opens documents, dispatches to registry, writes output |
...
main.py
  ...
  ├── core/pdf_converter.py → converters/base.py (get_format)
  │                         → utils/utils.py
...
- Never hardcode format keys in `menu.py` or `core/pdf_converter.py`.
